...
public class DapperOrderRepository(IConfiguration configuration) : IOrderRepository
{
    private readonly string connectionstring = configuration.GetConnectionString("Postgres");

    public OrderModel CreateOrder(OrderModel orderModelToCreate)
    {
        ...
        var orderId = connection.ExecuteScalar<int>(orderSql, orderModelToCreate);
        orderModelToCreate.OrderProducts.ForEach(op => op.OrderId = orderId);

        var orderProductSql = @"INSERT INTO storefront.orderproducts (
                                      orderid, productid, quantity, price) 
                                VALUES (@OrderId, @ProductId, @Quantity, @Price)";

        connection.Execute(orderProductSql, orderModelToCreate.OrderProducts);
        tran.Commit();
    ...
    public OrderModel? GetOrder(int id)
    {
        ...
        var result = connection.Query<OrderModel, CustomerModel, OrderProductModel, ProductModel, OrderModel>(
            sql, 
            param: new { Id = id },
